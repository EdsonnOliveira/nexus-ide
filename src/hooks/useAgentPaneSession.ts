...
import {
  ...
  replaceAgentStreamJsonSession,
  beginAgentStreamJsonIncompleteContinue,
  clearAgentStreamJsonIncompleteContinue,
} from '@/utils/agentStreamJsonSession';
...
import {
  completeAgentGitTurn,
  revertAgentGitChangesForTurn,
  trackAgentGitPrompt,
} from '@/utils/agentGitTurn';
import {
...
import {
  ...
  hasBlockingAgentToolWork,
  hasStreamingDevServerShell,
  scheduleAgentShellCommandHandoff,
...
import { hasAgentPromptAttachments } from '@/utils/agentPromptImageBadge';
...
import {
  ...
  hasPostToolFinalResponse,
  hasResumableStreamJsonTurnProgress,
  hasStreamJsonVisibleProgress,
  hydrateAgentStreamJsonSessionFromTurn,
...
import {
  trimAgentTurnHistory,
  sanitizeAgentTurnHistory,
  reuseStableAgentTurns,
...
import { wrapPromptWithAgentConversationHandoff } from '@/utils/buildAgentConversationHandoff';
...
const STREAM_JSON_EMPTY_HUNG_IDLE_MS = 600_000;
...
const STREAM_JSON_RECOVER_CONTINUE_MIN_TURN_MS = 3_000;
...
function takeConversationHandoffPrompt(
  prompt: string,
  turns: AgentTurn[],
  ...
  return wrapPromptWithAgentConversationHandoff(prompt, turns);
}
...
export function useAgentPaneSession({
  ...
  const streamJsonStateRef = useRef(getOrCreateAgentStreamJsonSession(tab.id));
  const cursorAgentContinueRef = useRef(false);
  const agentPrintRunActiveRef = useRef(false);
  const agentPrintRunTokenRef = useRef('');
  const preserveAgentShellChildrenRef = useRef(false);
  const resolvePreserveAgentPrintChildren = (force = false): boolean => {
    const flagged = preserveAgentShellChildrenRef.current;
    preserveAgentShellChildrenRef.current = false;
    const preserve =
      force ||
      flagged ||
      shouldKeepAgentShellsAlive(paneIdRef.current, streamJsonStateRef.current.activities);

    if (preserve) {
      scheduleAgentShellCommandHandoff(paneIdRef.current);
    }
  ...
  const bindAgentPrintRunToken = useCallback(
    (paneId: string, runToken: string, resetProgress = true) => {
  ...
  const syncOutputTailFromScrollback = useCallback(async () => {
    ...
    const tail = cleanAgentPtyChunk((scrollback ?? '').slice(-AGENT_OUTPUT_TAIL_SIZE)).replace(
      /\r/g,
      '\n',
    );
  ...
  useEffect(() => {
    ...
    turnsRef.current = incoming;
    setTurnsRevision((revision) => revision + 1);
  }, [isRuntimeActive, persistFollowUps, tab.turns]);
  ...
  const persistTurns = useCallback(
    (nextTurns: AgentTurn[], options?: { flush?: boolean }) => {
      ...
      if (options?.flush) {
        ...
        onTurnsChangeRef.current(stableTurns, { persist: false });
        flushPersistTurns();
        return;
      }
  ...
  useEffect(() => {
    ...
    if (!resolveCompatibleResumeChatId(resolveLiveCliAgent(tab.id, tab), boundResumeChatId)) {
      useTerminalSessionStore.getState().clearResumeChatId(tab.id);
      return;
    }
    ...
    setIsAgentReady(true);
  }, [resumeChatId, tab, tab.cliAgent, tab.id]);
  ...
  const finalizeActiveTurn = useCallback(
    (notifyOnComplete = false, options?: { force?: boolean }) => {
      ...
      if (usesStreamJson) {
        ...
        clearAgentPrintRunToken(paneId);
        finalizeMobileReleasesForPane(paneId, { failRemaining: true });
      } else {
        const outputSlice =
      ...
      completeAgentGitTurn(
        paneId,
        buildEditedFilesFromActivities(nextTurns[resolvedIndex]?.activities ?? []).map(
          (file) => file.path,
        ),
      );
  ...
  const finalizeStreamJsonTurnFromEvent = useCallback(
    (source = 'unknown') => {
      ...
      const turnStartedAt =
        [...turnsRef.current].reverse().find((turn) => turn.running)?.startedAt ?? null;
      ...
      const paneId = paneIdRef.current;
      const nextTurns = [...turns];
      nextTurns[resolvedIndex] = finalizeStreamJsonTurn(
        nextTurns[resolvedIndex]!,
  ...
  const tryFinalizeSettledStreamJsonTurn = useCallback(
    (processRunning = false, source = 'unknown') => {
      ...
      if (processRunning && hasStreamingDevServerShell(streamJsonStateRef.current.activities)) {
        return false;
      }
    ...
    [clearAgentPrintRunToken, finalizeStreamJsonTurnFromEvent, updateActiveTurn],
  );
  ...
  const tryHandoffLongRunningDevShell = useCallback((idleMs: number): boolean => {
    const state = streamJsonStateRef.current;

    if (hasStreamingDevServerShell(state.activities)) {
      return false;
    }

    if (hasPendingStreamJsonInteraction(state) || hasIncompleteStreamJsonTurnEnding(state)) {
      return false;
    }

    if (state.activities.some((entry) => entry.kind === 'thought' && entry.streaming)) {
      return false;
    }

    if (hasBlockingAgentToolWork(state.activities)) {
      return false;
    }
    ...
    if (idleMs < STREAM_JSON_READY_SHELL_HANDOFF_MS) {
      return false;
    }

    preserveAgentShellChildrenRef.current = true;
    scheduleAgentShellCommandHandoff(paneIdRef.current);
    state.handoffComplete = true;
    state.shouldFinalize = true;
    writeDebugSessionLog({
      location: 'useAgentPaneSession.ts:tryHandoff',
  ...
  const tryFinalizeCompleteStreamJsonResponse = useCallback(
    (idleMs: number, source: string): boolean => {
      if (idleMs < STREAM_JSON_COMPLETE_RESPONSE_IDLE_FINALIZE_MS) {
        return false;
      }
      ...
      if (hasIncompleteStreamJsonTurnEnding(state) || !hasPostToolFinalResponse(state)) {
        return false;
      }

      preserveAgentShellChildrenRef.current =
      ...
      clearAgentPrintRunToken(paneIdRef.current);
      finalizeStreamJsonTurnFromEvent(source);
      return true;
    },
    [clearAgentPrintRunToken, finalizeStreamJsonTurnFromEvent],
  );
  ...
  const scheduleStreamJsonSettleCheck = useCallback(() => {
      ...
      void window.nexus.agentPrint.isRunning(paneIdRef.current).then((processRunning) => {
        ...
        if (processRunning) {
          ...
          if (tryHandoffLongRunningDevShell(idleMs)) {
            ...
            clearAgentPrintRunToken(paneIdRef.current);
            finalizeStreamJsonTurnFromEvent('settleTimer-handoff');
            return;
          }

          if (tryFinalizeCompleteStreamJsonResponse(idleMs, 'settleTimer-complete-response')) {
            return;
          }
    ...
    tryFinalizeCompleteStreamJsonResponse,
    ...
    (chunk: string): boolean => {
      ...
      if (streamUpdate.sessionId) {
        ...
        if (
          ...
          resolveCompatibleResumeChatId(
            streamJsonRunCliRef.current || resolveLiveCliAgent(paneIdRef.current, tab),
            streamUpdate.sessionId,
          )
        ) {
          useTerminalSessionStore
            .getState()
  ...
  const startStreamJsonAgentRun = useCallback(
    (
      ...
      const rawResumeChatId =
        session.resumeChatIdByPane[paneId]?.trim() ||
        (allowTurnResume ? lastTurnResumeChatId : null);
      const compatibleResumeChatId = resolveCompatibleResumeChatId(cliAgent, rawResumeChatId);
      const droppedIncompatibleResume = Boolean(rawResumeChatId) && !compatibleResumeChatId;

      if (modeChangedSinceLastPrint || forceFreshSession || droppedIncompatibleResume) {
        session.clearResumeChatId(paneId);
        cursorAgentContinueRef.current = false;
      }

      if (droppedIncompatibleResume) {
        turnsRef.current = turnsRef.current.map((turn) =>
          turn.resumeChatId ? { ...turn, resumeChatId: null } : turn,
        );
      }
      ...
      const resumeChatId =
        modeChangedSinceLastPrint || forceFreshSession || droppedIncompatibleResume
          ? null
          : compatibleResumeChatId;
      const hasCompletedTurn = turnsRef.current.some(
        (turn) => !turn.running && !turn.pendingFollowUp,
      );
      const cliChanged = Boolean(lastRunCli && !sameCliAgent(lastRunCli, cliAgent));
      const continueSession =
        !cliChanged &&
        !droppedIncompatibleResume &&
        !forceFreshSession &&
        !resumeChatId &&
      ...
      const promptForRun =
        prompt.trim() &&
        prompt.trim() !== STREAM_JSON_INCOMPLETE_CONTINUE_PROMPT &&
        !resumeChatId &&
        !continueSession
          ? wrapPromptWithAgentConversationHandoff(prompt, turnsRef.current)
          : prompt;
      ...
      if (!options?.isAutoRetry) {
        ...
        streamJsonIncompleteContinueRef.current = false;
        streamJsonIncompleteContinueCountRef.current = 0;
        clearAgentStreamJsonIncompleteContinue(paneId);
      }
  ...
  const tryContinueIncompleteStreamJsonTurn = useCallback(
    (finishAgentPrintRun: () => void): boolean => {
      ...
      if (streamJsonIncompleteContinueCountRef.current >= STREAM_JSON_MAX_INCOMPLETE_CONTINUES) {
        return false;
      }

      const runningTurn = [...turnsRef.current].reverse().find((turn) => turn.running);
      ...
      const resumeChatId = resolveCompatibleResumeChatId(
        continueCli,
        streamJsonStateRef.current.sessionId?.trim() ||
          session.resumeChatIdByPane[paneId]?.trim() ||
          runningTurn.resumeChatId?.trim() ||
          null,
      );

      hydrateAgentStreamJsonSessionFromTurn(streamJsonStateRef.current, runningTurn, resumeChatId);

      if (
        !hasIncompleteStreamJsonTurnEnding(
          streamJsonStateRef.current,
          streamJsonStateRef.current.activities.length > 0
            ? streamJsonStateRef.current.activities
      ...
      const canContinueWithoutId =
        ...
        (cursorAgentContinueRef.current ||
          ...
          hasResumableStreamJsonTurnProgress(runningTurn));

      if (!resumeChatId && !canContinueWithoutId) {
        return false;
      }

      if (!beginAgentStreamJsonIncompleteContinue(paneId)) {
        return false;
      }

      streamJsonIncompleteContinueRef.current = true;
      streamJsonIncompleteContinueCountRef.current += 1;
      streamJsonStateRef.current.shouldFinalize = false;
      ...
      const continueTurnId = runningTurn.id;
  ...
  const runCommand = useCallback(
    (command: string) => {
      ...
      if (aiProvider) {
        ...
        conversationHandoffPendingRef.current = turnsRef.current.some(
          (turn) => !turn.running && !turn.pendingFollowUp,
        );
        const nextTurns = turnsRef.current.map((turn) =>
          turn.resumeChatId ? { ...turn, resumeChatId: null } : turn,
        );
        if (nextTurns.some((turn, index) => turn !== turnsRef.current[index])) {
          turnsRef.current = nextTurns;
          persistTurns(nextTurns, { flush: true });
        }
    ...
    [persistTurns, scheduleSetupSettled, tab, usesStreamJson, writeToPty],
  );

  const stopAgent = useCallback(
    (options?: { preserveFollowUps?: boolean; preserveShellTerminals?: boolean }) => {
      const ptyId = ptyIdRef.current;
      const paneId = paneIdRef.current;
      const session = useTerminalSessionStore.getState();
      ...
      if (usesStreamJson) {
        window.nexus.agentPrint.stop(paneId, {
          preserveChildren: resolvePreserveAgentPrintChildren(
            Boolean(options?.preserveShellTerminals),
          ),
        });
      }

      if (ptyId && !options?.preserveShellTerminals) {
      ...
      completeAgentGitTurn(
        paneId,
        latestTurn
          ? buildEditedFilesFromActivities(latestTurn.activities).map((file) => file.path)
          : undefined,
      );
  ...
  const sendPromptToPty = useCallback(
    (trimmed: string, imageRefs: string[]) => {
      ...
      const paneId = paneIdRef.current;
      const session = useTerminalSessionStore.getState();
      const promptForPty = takeConversationHandoffPrompt(
        trimmed,
        turnsRef.current,
        conversationHandoffPendingRef,
      );

      writeToPty(PTY_CLEAR_INPUT);

      if (promptForPty) {
        session.setLastCommand(paneId, promptForPty);
        trackAgentGitPrompt(paneId, promptForPty);

        if (shouldMarkAgentAwaiting(paneId, promptForPty, session.activeAgentByPane)) {
          session.markAwaitingResponse(paneId);
        }
      ...
      if (promptForPty || imageRefs.length > 0) {
        writeToPty('\n');
        recordHomeDashboardActivity('prompts');
      }
    },
      ...
      try {
        if (hasRunningTurn) {
          if (usesStreamJson) {
            ...
            const runningActivities = turnsRef.current.flatMap((turn) =>
              turn.running ? turn.activities : [],
            );
            const preserveShells = shouldKeepAgentShellsAlive(paneId, runningActivities);
            preserveAgentShellChildrenRef.current = preserveShells;
            stopAgent({ preserveFollowUps: true, preserveShellTerminals: preserveShells });
          } else {
            const session = useTerminalSessionStore.getState();
            const promptForPty = takeConversationHandoffPrompt(
              agentPrompt,
  ...
  const tryFlushFollowUpQueue = useCallback(
    (options?: { force?: boolean; onlyId?: string }): boolean => {
      if (followUpFlushInFlightRef.current || suppressFollowUpFlushRef.current) {
        return false;
      }

      const queue = followUpsRef.current.filter(
      ...
      const target = options?.onlyId ? queue.find((item) => item.id === options.onlyId) : queue[0];

      if (!target) {
        return false;
      }

      if (consumedFollowUpIdsRef.current.has(target.id)) {
        return false;
      }
  ...
  const submitPrompt = useCallback(
    async (prompt: string, options?: AgentPromptSubmitOptions): Promise<boolean> => {
      ...
      if (hasRunningTurn && !editingTurnId && !options?.forceNewTurn) {
        ...
        return enqueueFollowUp(trimmed, attachments, targetAiProvider);
      }
      ...
      try {
        ...
        if (targetAiProvider) {
          runCommand(formatAgentAiProviderCommand(targetAiProvider));
        }
          ...
          if (usesStreamJson && turnsRef.current.some((turn) => turn.running)) {
            updateActiveTurn((turn) => ({
              ...
              activities: [
                ...
                createFailedPromptActivity('Envio cancelado antes de iniciar o agent.'),
              ],
            }));
            finalizeActiveTurn(true, { force: true });
          }
  ...
  const acceptPlan = useCallback(
    async (activityId: string): Promise<boolean> => {
      ...
      const submitted = await submitPrompt(
        buildAgentPlanImplementPrompt(planBody, activity.planName),
      );
  ...
  const failStuckTurn = useCallback(() => {
    ...
    const withoutThought = activeTurn.activities.filter(
      (entry) =>
        entry.kind !== 'thought' && entry.kind !== 'live_status' && entry.kind !== 'tool_run',
    );
  ...
  useEffect(() => {
    ...
    paneIdRef.current = tab.id;
    const sessionState = getOrCreateAgentStreamJsonSession(tab.id);
    streamJsonStateRef.current = sessionState;
    const runningTurn = turnsRef.current.find((turn) => turn.running);
    const storedResumeChatId = resolveCompatibleResumeChatId(
      resolveLiveCliAgent(tab.id, tab),
      useTerminalSessionStore.getState().resumeChatIdByPane[tab.id]?.trim() ||
        runningTurn?.resumeChatId?.trim() ||
    ...
    if (runningTurn) {
      hydrateAgentStreamJsonSessionFromTurn(sessionState, runningTurn, storedResumeChatId);
    } else {
      sessionState.sessionId = storedResumeChatId;
    }

    if (
      sessionState.activities.length > 0 ||
      sessionState.pendingResponseText.trim().length > 0 ||
      sessionState.jsonBuffer.trim().length > 0 ||
      Boolean(runningTurn && hasResumableStreamJsonTurnProgress(runningTurn))
    ) {
      hasStreamJsonChunkRef.current = true;
      cursorAgentContinueRef.current = true;
      lastStreamJsonChunkAtRef.current = Date.now();
    ...
    if (storedResumeChatId) {
      cursorAgentContinueRef.current = true;
      useTerminalSessionStore.getState().setResumeChatId(tab.id, storedResumeChatId);
    }
  ...
  useEffect(() => {
    ...
    const recover = async () => {
      ...
      try {
        const adopt = window.nexus.agentPrint.adopt;
        snapshot = adopt
          ? await adopt(paneId)
          : {
              running: await window.nexus.agentPrint.isRunning(paneId),
              runToken: resolveAgentPrintRunToken(paneId) || null,
              exit: null,
            };
      } catch {
        snapshot = {
          running: await window.nexus.agentPrint.isRunning(paneId).catch(() => false),
          runToken: resolveAgentPrintRunToken(paneId) || null,
          exit: null,
        };
      }
      ...
      if (snapshot.runToken) {
        bindAgentPrintRunToken(paneId, snapshot.runToken, false);
      }
      ...
      if (Date.now() - runningTurn.startedAt < STREAM_JSON_RECOVER_CONTINUE_MIN_TURN_MS) {
        return;
      }
  ...
  const clearStaleAgentPty = useCallback(
    async (currentPtyId: string) => {
  ...
  useEffect(() => {
    ...
    void window.nexus.terminal
      .getScrollbackTail(ptyId, AGENT_OUTPUT_TAIL_SIZE)
      .then((scrollback) => {
        if (cancelled) {
          return;
        }
  ...
  useEffect(() => {
    ...
    const unsubscribeData = window.nexus.terminal.onData((incomingPtyId, data) => {
      ...
      if (cleaned) {
        ...
        if (!hasRunningTurn && detectAgentFollowUpReadyInChunk(outputTailRef.current)) {
          useTerminalSessionStore.getState().resetAgentWorkload(paneId);
        }
  ...
  useEffect(() => {
    ...
    return registerAgentPrintPaneHandlers(paneId, {
      onData: (incomingPaneId, data, runToken) => {
        ...
        let expectedToken = resolveAgentPrintRunToken(paneIdRef.current);
        ...
        if (
          ...
          bindAgentPrintRunToken(paneIdRef.current, runToken, false);
        ...
        if (!agentPrintRunActiveRef.current && !turnsRef.current.some((turn) => turn.running)) {
          return;
        }
      ...
      onDone: (incomingPaneId, payload) => {
        ...
        if (
          ...
          bindAgentPrintRunToken(paneIdRef.current, payload.runToken, false);
          expectedToken = payload.runToken;
        }
  ...
  useEffect(() => {
    if (!usesStreamJson || !isTurnRunning || !isRuntimeActive) {
      return;
    }
    ...
    const intervalId = window.setInterval(() => {
      ...
      void window.nexus.agentPrint.isRunning(paneId).then((processRunning) => {
        if (processRunning) {
          ...
          const hasBlockingToolWork = hasBlockingAgentToolWork(
            streamJsonStateRef.current.activities,
          );
          const keepDevServerProcess =
            hasBlockingToolWork ||
            hasStreamingDevServerShell(streamJsonStateRef.current.activities);
          ...
          if (tryFinalizeCompleteStreamJsonResponse(idleMs, 'idleInterval-complete-response')) {
            return;
          }

          if (keepDevServerProcess && idleMs < STREAM_JSON_ACTIVE_TOOL_HUNG_IDLE_MS) {
            if (idleMs >= STREAM_JSON_STALL_UI_MS) {
              syncStreamJsonStallLiveStatus(idleMs);
            }
        ...
        const orphanThreshold = missingResponse
