...
function resolveInlineCodeBadgeTone(raw: string): InlineCodeBadgeTone {
  ...
  if (
    token.includes('/') ||
    /\.(?:tsx?|jsx?|mjs|cjs|py|go|rs|sql|json|yaml|yml|md)$/i.test(token)
  ) {
    return 'path';
  }
  ...
  if (
    /^(import|export|from|const|let|var|return|async|await|if|else|for|while|class|interface|type|enum)$/i.test(
      token,
    )
  ) {
    return 'keyword';
...
function resolveInlineCodeBadgeClass(raw: string): string {
